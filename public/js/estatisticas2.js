...
async function forcarSincronizacao(btn) {
  ...
  btn.innerHTML =
    '<span class="spinner-border spinner-border-sm"></span> Baixando 2026...';
  btn.disabled = true;
  await carregarEstatisticas(false);
...
async function carregarEstatisticas(forcar = false) {
  ...
  board.innerHTML =
    '<div class="text-center w-100 mt-5 pt-5 text-white-50 fw-bold">Processando dados de 2026...</div>';

  try {
    // Acessa a rota livre de autenticação para buscar a Base Premium Nativa
    const url = "/api/elenco?t=" + Date.now();
...
function renderizarGrafico() {
  ...
  // Ordena pelos que tem mais jogos
  ...
  // Ajusta a altura da div do canvas baseada na quantidade de jogadores (evita barras espremidas)
  ...
  const backgroundColors = lista.map((j) => {
    ...
    if (p === "G") return "#f59e0b"; // Amarelo (Goleiro)
    if (p === "D") return "#3b82f6"; // Azul (Defesa)
    if (p === "M") return "#10b981"; // Verde (Meio)
    if (p === "F" || p === "A") return "#ef4444"; // Vermelho (Ataque)
    return "#ffffff";
  });
...
function renderizarCampoTatico() {
  ...
  // INJEÇÃO DE SEGURANÇA: Garante que o campo de futebol seja desenhado mesmo que ocorra cache de CSS no navegador
  if (!document.getElementById("style-campo-tatico-force")) {
    ...
    style.innerHTML = `
      ...
      .jogador-foto { transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275) !important; z-index: 2 !important; }
      .jogador-nome { transition: all 0.3s ease !important; z-index: 3 !important; }
    `;
  ...
  const normalizePos = (p) => {
    ...
    if (
      ...
      return "F";
    return "M"; // Fallback genérico para Meio-Campo se vier nulo
  };
...
function renderizarListas() {
  ...
  boxGols.innerHTML = artilheiros.length
    ...
    : '<p class="text-white-50 small mt-2">Sem gols oficiais na base.</p>';
  boxAssist.innerHTML = garcons.length
    ? garcons.map((j, i) => renderRow(j, i, j.assistencias, "Ast")).join("")
    : '<p class="text-white-50 small mt-2">Sem assistências oficiais na base.</p>';
}
